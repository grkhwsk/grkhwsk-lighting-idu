...
Запуск: python scripts/passport_export.py
"""
...
OUTPUT_FILE = 'data/output/passport_s0.xlsx'
...
def p24h_fill(val):
    """Окраска по % постоянной потребности."""
    if not isinstance(val, (int, float)) or pd.isna(val):
        return hfill("FFFFFF")
    if val >= 50:  return hfill("FFD6D6")
...
gdf['pct_24h']       = gdf['pct_24h'].fillna(0) if 'pct_24h' in gdf.columns else 0
...
w1 = [24, 12, 14, 12, 20, 22]
header_row(ws1, 1, h1, w1)
...
for zone in zones_present:
    ...
    if zdf.empty:
        continue
    mask_avg  = round(zdf['mask_pct'].mean(), 1)
    p24h_avg  = round(zdf['pct_24h'].mean(), 1) if 'pct_24h' in gdf.columns else 0.0
    area_km2  = round(zdf['area_ha'].sum() / 100, 2)
    poi_total = int(zdf['poi_count'].sum())

    row_vals = [zone, len(zdf), area_km2, poi_total, mask_avg, p24h_avg]
    for ci, val in enumerate(row_vals, 1):
        ...
        if ci == 6:
            fill = p24h_fill(val)
            nfmt = '0.0'
        write_cell(ws1, ri, ci, val, bold=bold, center=center,
...
poi_type_labels = [TYPE_LABELS.get(f.replace('poi_', ''), f.replace('poi_', ''))
...
w2 = ([22, 12, 10, 12, 12, 26, 20, 22] +
      [13] * len(poi_type_fields))
header_row(ws2, 1, h2, w2)
...
for zone in zones_present:
    ...
    for _, row in zdf.iterrows():
        ...
        p24h = round(float(row['pct_24h']), 1) if 'pct_24h' in gdf.columns else 0.0
        ...
        for ci, val in enumerate(row_vals, 1):
            ...
            if ci == 8:
                fill = p24h_fill(val)
                nfmt = '0.0'
            write_cell(ws2, ri, ci, val, center=center,
                       fill=fill, num_fmt=nfmt)
        ri += 1
...
h3 = ['Зона ПЗЗ', 'Тип активности', 'Объектов rank 1', 'Доля в зоне, %']
w3 = [24, 30, 18, 16]
header_row(ws3, 1, h3, w3)
...
for zone in zones_present:
    ...
    if zdf.empty:
        continue
    type_totals = {f.replace('poi_', ''): int(zdf[f].fillna(0).sum())
                   for f in poi_type_fields}
